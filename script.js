function iniciarSimulacao() {
    ...
    while (continuar === true) {
        ...
        let indiceProduto = 0; // Antes era 'i'
        while (indiceProduto < produtos.length) {
            lista += (indiceProduto + 1) + " - " + produtos[indiceProduto][0] + " (R$ " + produtos[indiceProduto][1].toFixed(2) + ")\n";
            indiceProduto++;
        }
        ...
        // Adiciona produto ao carrinho (usando a função criada)
        adicionarAoCarrinho(carrinho, produtos[escolha - 1]);
    }

    // Calcula subtotal, desconto e total (usando a função criada)
...
function calcularTotal(carrinho) {
    if (carrinho.length > 0) {
        ...
        while (indiceCarrinho < carrinho.length) {
            subtotal += carrinho[indiceCarrinho][1];
            indiceCarrinho++;
        }
        ...
        indiceCarrinho = 0; 
        while (indiceCarrinho < carrinho.length) {
            resumo += "- " + carrinho[indiceCarrinho][0] + " (R$ " + carrinho[indiceCarrinho][1].toFixed(2) + ")\n";
            indiceCarrinho++;
        }
